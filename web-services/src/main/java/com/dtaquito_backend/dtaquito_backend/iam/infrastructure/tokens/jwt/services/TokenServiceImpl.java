...
public class TokenServiceImpl implements BearerTokenService {
    ...
    public String generateToken(Authentication authentication) {
        ...
        String token = buildTokenWithDefaultParameters(authentication.getPrincipal().toString());
        LOGGER.debug("Generated token: {}", token);
        LOGGER.info("Complete JWT: {}", token); // Agrega esta línea
        return token;
    }

    public String generateToken(String username) {
        LOGGER.debug("Generating token for username: {}", username);
        String token = buildTokenWithDefaultParameters(username);
        LOGGER.debug("Generated token: {}", token);
        return token;
    }
    ...
    public String getBearerTokenFrom(HttpServletRequest request) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            LOGGER.info("Received JWT: {}", token); // Agrega esta línea
            return token;
        }
        return null;
    }
